# Vendored from https://github.com/mfg92/hantek1008py (commit as of 2026-08-24),
# licensed Apache License 2.0 -- see LICENSE in this directory.
#
# Modifications from upstream (per Apache 2.0 section 4(b), changes are
# noted here rather than inline at every call site):
#   - _init3(): four hardcoded `assert response == <bytes>` checks against
#     factory calibration data (per-unit EEPROM contents, not a protocol
#     constant) were changed to warning logs instead of hard failures.
#     Confirmed against a real 1008C unit on 2026-08-25 -- see
#     ../../../../docs/hantek1008c.md for the full bring-up writeup.
#   - __calc_correction_factor(): fixed a real upstream bug. When a
#     channel/vscale has exactly one correction_data point (the normal
#     case for our calibration flow -- the 0V point is intentionally
#     excluded, same as upstream csvexport.py does), the original code
#     did `channel_cd[0]`, which indexes the correction dict (keyed by
#     raw ADC delta values, e.g. 1673.19) as if it were a list -- a
#     guaranteed KeyError unless a point happens to have delta exactly
#     0. Changed to `next(iter(channel_cd.values()))` to just take the
#     single value regardless of its key. Found 2026-08-25 running our
#     own calibration script for the first time.
# Do not hand-edit this file beyond re-applying these patches when
# pulling upstream updates; app-specific logic belongs in driver.py.
...
class Hantek1008Raw:
    ...
    def _init3(self) -> None:
        ...
        if response != bytes.fromhex("d606"):
            log.warning(f"0xe5 calibration response differs from upstream reference unit: {response.hex()}")

        response = self.__send_cmd(0xf7, echo_expected=False, response_length=64)
        if response != bytes.fromhex("2cfd8ffb54fa2ef878007a007b00780079007a0079007800b801bf01c301ba01"
                                      "bb01be01b701b801f90203030803fb02fc020003f502f80294ff92ff8fff93ff"):
            log.warning(f"0xf7 calibration response differs from upstream reference unit: {response.hex()}")

        response = self.__send_cmd(0xf8, echo_expected=False, response_length=64)
        if response != bytes.fromhex("92ff91ff96ff94ffc9fec4febdfec8fec7fec2fecffec9fe4cfe45fe3afe4afe"
                                      "48fe42fe54fe4dfe70ff70ff71ff70ff71ff71ff72ff71ff7efe7bfe7afe7efe"):
            log.warning(f"0xf8 calibration response differs from upstream reference unit: {response.hex()}")

        response = self.__send_cmd(0xfa, echo_expected=False, response_length=56)
        if response != bytes.fromhex("7dfe7efe80fe7ffe90019401930192018f01900191018f0195029b0299029802"
                                      "930294029702940290fd89fd84fd90fd8dfd8cfd94fd91fd"):
            log.warning(f"0xfa calibration response differs from upstream reference unit: {response.hex()}")
...
class Hantek1008(Hantek1008Raw):
    ...
    def __calc_correction_factor(self, delta_to_zero: float, channel_id: int, vscale: float) -> float:
        ...
        if len(channel_cd) == 1:
            return next(iter(channel_cd.values()))
